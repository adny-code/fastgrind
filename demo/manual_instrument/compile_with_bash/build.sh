...
compile_pkgA() {
  ...
  mkdir -p "$(dirname "$obj")"
  $CXX ${CXXFLAGS} "${INCLUDE_FLAGS[@]}" -I"$SCRIPT_DIR/pkgA" -c "$src" -o "$obj"
...
compile_pkgB() {
  ...
  $CXX ${CXXFLAGS} "${INCLUDE_FLAGS[@]}" -I"$SCRIPT_DIR/pkgA" -I"$SCRIPT_DIR/pkgB" -c "$src" -o "$obj"
...
compile_pkgC() {
  ...
  $CXX ${CXXFLAGS} "${INCLUDE_FLAGS[@]}" -I"$SCRIPT_DIR/pkgC" -c "$src" -o "$obj"

...
void testMetaprogramming()
{
    ...
    std::cout << "Compile-time Factorial<12> = " << compile_time_result << " (measurement overhead only)" << std::endl;

    std::function<int(int)> runtime_factorial = [](int n) -> int {
        std::function<int(int)> factorial_impl;
        factorial_impl = [&factorial_impl](int x) -> int { return x <= 1 ? 1 : x * factorial_impl(x - 1); };
    ...
    std::cout << "Runtime factorial(12) = " << runtime_result << " (took " << duration.count() << " ns)" << std::endl;
}
...
void testDynamicBinding()
{
    ...
    binder.bind("lambda", []() { std::cout << "Lambda function executed!" << std::endl; });

    auto regularFunc = []() { std::cout << "Regular function executed!" << std::endl; };
    binder.bind("regular", regularFunc);

    int counter = 0;
    binder.bind("counter",
                [&counter]() { std::cout << "Counter function executed! Count: " << ++counter << std::endl; });

    std::string message = "Hello from member function!";
    binder.bind("member", [&message]() { std::cout << message << std::endl; });
